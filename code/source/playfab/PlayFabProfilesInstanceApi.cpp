...
namespace PlayFab
{
    ...
    void PlayFabProfilesInstanceAPI::GetTitlePlayersFromXboxLiveIDs(
        GetTitlePlayersFromXboxLiveIDsRequest& request,
        const ProcessApiCallback<GetTitlePlayersFromProviderIDsResponse> callback,
        const ErrorCallback errorCallback,
        void* customData
    )
    {
        ...
        auto reqContainer = std::unique_ptr<CallRequestContainer>(new CallRequestContainer(
            ...
            headers,
            jsonAsString,
            std::bind(&PlayFabProfilesInstanceAPI::OnGetTitlePlayersFromXboxLiveIDsResult, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
            settings,
            context,
            customData));

        reqContainer->successCallback = std::shared_ptr<void>((callback == nullptr) ? nullptr : new ProcessApiCallback<GetTitlePlayersFromProviderIDsResponse>(callback));
        reqContainer->errorCallback = errorCallback;

        http.MakePostRequest(std::unique_ptr<CallRequestContainerBase>(static_cast<CallRequestContainerBase*>(reqContainer.release())));
    }

    void PlayFabProfilesInstanceAPI::OnGetTitlePlayersFromXboxLiveIDsResult(int /*httpCode*/, const std::string& /*result*/, const std::shared_ptr<CallRequestContainerBase>& reqContainer)
    {
        CallRequestContainer& container = static_cast<CallRequestContainer&>(*reqContainer);
        std::shared_ptr<PlayFabAuthenticationContext> context = container.GetContext();

        GetTitlePlayersFromProviderIDsResponse outResult;
        if (ValidateResult(outResult, container))
        {
            std::shared_ptr<void> internalPtr = container.successCallback;
            if (internalPtr.get() != nullptr)
            {
                const auto& callback = *static_cast<ProcessApiCallback<GetTitlePlayersFromProviderIDsResponse> *>(internalPtr.get());
                callback(outResult, container.GetCustomData());
            }
        }
    }
    ...
    void PlayFabProfilesInstanceAPI::OnSetDisplayNameResult(int /*httpCode*/, const std::string& /*result*/, const std::shared_ptr<CallRequestContainerBase>& reqContainer)
    {
        ...
        if (ValidateResult(outResult, container))
        {
            ...
            if (internalPtr.get() != nullptr)
            {
                const auto& callback = *static_cast<ProcessApiCallback<SetDisplayNameResponse> *>(internalPtr.get());
                callback(outResult, container.GetCustomData());
            }
        }
    }
